<template>
    <div>
        ...
        <el-dialog title="患者信息" v-model="data.formVisible" destory-on-close width="750">
...
const add = () => {
    request.post('/patient/add', data.form).then(res => {
        if (res.code === '200') {
...
const update = () => {
    request.put('/patient/update', data.form).then(res => {
        if (res.code === '200') {
            data.formVisible = false
            ElMessage.success('操作成功')
            pageStore.loadData()
...
const handleDelete = (id) => {
    ElMessageBox.confirm('删除后的数据无法恢复，您确认删除吗', '删除确认', { type: 'warning' }).then(() => {
        request.delete('/patient/deleteById/' + id).then(res => {
            if (res.code === '200') {
                ElMessage.success('操作成功')
                pageStore.loadData()
            } else {
...
const delBatch = () => {
    ...
    ElMessageBox.confirm('删除后的数据无法恢复，您确认删除吗', '删除确认', { type: 'warning' }).then(() => {
        request.delete('/patient/delBatch', { data: data.ids }).then(res=>{
            if(res.code==='200'){
                ElMessage.success('操作成功')
                pageStore.loadData()
            }else{
